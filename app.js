...
const filterReadCheckbox = document.getElementById("filter-read");
const filterNotReadCheckbox = document.getElementById("filter-not-read");

filterReadCheckbox.addEventListener("change", updateBookDisplay);
filterNotReadCheckbox.addEventListener("change", updateBookDisplay);

function updateBookDisplay() {
	const showRead = filterReadCheckbox.checked;
	const showNotRead = filterNotReadCheckbox.checked;

	const filteredBooks = myLibrary.filter((book) => {
		if (showRead && showNotRead) {
			return true;
		} else if (showRead) {
